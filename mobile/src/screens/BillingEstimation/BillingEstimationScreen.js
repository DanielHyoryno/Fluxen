...
export default function BillingEstimationScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [categoriesResp, devicesResp] = await Promise.all([listCategoriesApi(token), listDevicesApi(token)]);
    ...
    <ScrollView style={styles.page} contentContainerStyle={styles.content}>
      ...
      <View style={styles.heroCard}>
        ...
        <View style={styles.heroRangeBlock}>
          <View style={styles.presetRow}>
            {[
              ...
              >
                <Text style={[styles.presetButtonText, rangePreset === item.key && styles.presetButtonTextActive]}>{item.label}</Text>
              </Pressable>
            ))}
          </View>
        ...
        <View style={styles.heroStats}>
          <View style={styles.heroStatBox}>
            ...
            <Text style={styles.heroStatValue}>{Number(estimate?.summary?.total_liters || 0).toFixed(3)} L</Text>
          </View>
          <View style={styles.heroStatBox}>
      ...
      <View style={styles.card}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
          ...
          >
            <Text style={[styles.chipText, selectedCategoryId === null && styles.chipTextActive]}>{messages.billing.showAll}</Text>
          </Pressable>
          {categories.map((category) => (
            ...
            >
              <Text style={[styles.chipText, selectedCategoryId === category.id && styles.chipTextActive]}>{category.name}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </View>
      ...
      <View style={styles.card}>
        ...
        {filteredDevices.map((device) => {
          ...
          return (
            <Pressable key={device.id} style={[styles.deviceRow, checked && styles.deviceRowChecked]} onPress={() => toggleDevice(device.id)}>
              <View style={[styles.checkbox, checked && styles.checkboxChecked]} />
              <View style={styles.deviceInfo}>
                <Text style={styles.deviceName}>{formatDeviceLabel(device.device_name, device.install_location)}</Text>
        ...
        <Pressable style={[styles.button, estimating && styles.buttonDisabled]} onPress={handleApply} disabled={estimating}>
          {estimating ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>{messages.billing.apply}</Text>}
        </Pressable>
      </View>
          ...
          <View style={styles.resultList}>
            {(estimate.items || []).map((item) => (
              <View key={item.device_id} style={styles.resultRow}>
                <View style={styles.deviceInfo}>
                  <Text style={styles.deviceName}>{formatDeviceLabel(item.device_name, item.install_location)}</Text>
      ...
      <Modal transparent visible={pickerVisible} animationType="fade" onRequestClose={() => setPickerVisible(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{pickerTarget === "from" ? messages.billing.pickStartDate : messages.billing.pickEndDate}</Text>
            <Calendar
            ...
            />
            {Platform.OS === "web" ? <Text style={styles.helperText}>{messages.billing.webDateHint}</Text> : null}
